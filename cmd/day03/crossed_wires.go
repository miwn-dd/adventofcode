...
func getCentralPort() point {
	// lower left corner
	return point{gridSize / 2, gridSize / 2}
}

func (g *grid) initGrid() {
	g.array = make([][]byte, gridSize)

	for row := 0; row < gridSize; row++ {
		g.array[row] = make([]byte, gridSize)
...
func (g *grid) drawWire(path string, token byte) {
	// parse path
	directions := strings.Split(path, ",")

	rowStart, colStart := getCentralPort().row, getCentralPort().col

	for _, direction := range directions {
		distance, _ := strconv.Atoi(string(direction[1:]))

		switch direction[0] {
		case 'U':
			rowStart, colStart = g.drawDirection(rowStart, colStart, distance, 0, -1, token)
		case 'D':
			rowStart, colStart = g.drawDirection(rowStart, colStart, distance, 0, 1, token)
		case 'R':
			rowStart, colStart = g.drawDirection(rowStart, colStart, 0, distance, 1, token)
		case 'L':
			rowStart, colStart = g.drawDirection(rowStart, colStart, 0, distance, -1, token)
		}
	}
...
func (g *grid) drawDirection(rowStart, colStart, deltaRow, deltaCol, direction int, token byte) (int, int) {
	if deltaRow > 0 {
		for index := 1; index <= deltaRow; index++ {
			g.setCell(rowStart+index*direction, colStart, token)
		}

		return rowStart + deltaRow*direction, colStart

	}

	for index := 1; index <= deltaCol; index++ {
		g.setCell(rowStart, colStart+index*direction, token)
	}

	return rowStart, colStart + deltaCol*direction
}

func (g *grid) setCell(row, col int, token byte) {
	currentCellValue := g.array[row][col]

	if currentCellValue == 'X' {
		return
	}

	// mark crossing if cell is not empty and cell is not filled with same wire token
	if currentCellValue != '.' && currentCellValue != token {
		g.array[row][col] = 'X'
	} else {
		g.array[row][col] = token
	}
}

func (g grid) findDistanceToNearestIntersection() int {
	centralPort := getCentralPort()

	distance := float64(gridSize * 2)

	for row := range g.array {
		for col := range g.array[row] {
			if g.array[row][col] == 'X' {
				newDist := math.Abs(float64(centralPort.row-row)) + math.Abs(float64(centralPort.col-col))
...
func (g grid) findStepsToNearestIntersection(wire1, wire2 string) int {
	centralPort := getCentralPort()
	...
	minimumSteps := gridSize * gridSize
	for i := range intersections {
		intersection := intersections[i]

		steps1 := countStepsForWire(wire1, centralPort, intersection)
		steps2 := countStepsForWire(wire2, centralPort, intersection)

		if steps1+steps2 < minimumSteps {
			minimumSteps = steps1 + steps2
		}
...
func (g grid) printGrid() {
	for row := range g.array {
		fmt.Println(g.array[row])
	}
}

func countStepsForWire(wire string, start, target point) int {
	// parse path
	directions := strings.Split(wire, ",")

	steps := 0
	targetReached := false
	currentPos := point{start.row, start.col}
	for _, direction := range directions {
		distance, _ := strconv.Atoi(string(direction[1:]))

		switch direction[0] {
		case 'U':
			steps, targetReached, currentPos = countStepsForDirection(currentPos, target, distance, 0, -1, steps)
		case 'D':
			steps, targetReached, currentPos = countStepsForDirection(currentPos, target, distance, 0, 1, steps)
		case 'R':
			steps, targetReached, currentPos = countStepsForDirection(currentPos, target, 0, distance, 1, steps)
		case 'L':
			steps, targetReached, currentPos = countStepsForDirection(currentPos, target, 0, distance, -1, steps)
		}
...
func countStepsForDirection(start, target point, deltaRow, deltaCol, direction, startSteps int) (steps int, targetReached bool, endPos point) {
	steps = startSteps
	targetReached = false
	endPos = point{start.row, start.col}

	if deltaRow > 0 {
		for index := 1; index <= deltaRow; index++ {
			steps++

			endPos.row += direction

			if target.row == start.row+index*direction && target.col == start.col {
				targetReached = true
				return
			}
		}
		...
		for index := 1; index <= deltaCol; index++ {
			...
			endPos.col += direction

			if target.row == start.row && target.col == start.col+index*direction {
				targetReached = true
				return
			}
		}
...
func puzzle1(wire1, wire2 string) int {
	var grid grid
	...
	return grid.findDistanceToNearestIntersection()
}

func puzzle2(wire1, wire2 string) int {
	var grid grid
	...
	return grid.findStepsToNearestIntersection(wire1, wire2)
}

func main() {
	fmt.Printf("Result puzzle1: %v\n", puzzle1(input1, input2))

	fmt.Printf("Result puzzle2: %v\n", puzzle2(input1, input2))
}
